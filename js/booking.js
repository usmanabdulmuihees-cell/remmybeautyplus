...
function go(step){state.step=step;steps.forEach(item=>item.classList.toggle('active',+item.dataset.step===step));document.querySelectorAll('.progress-labels span').forEach((item,index)=>item.classList.toggle('active',index<step));$('#progressFill').style.width=`${step*25}%`;$('#backBtn').style.visibility=step===1?'hidden':'visible';$('#nextBtn').classList.toggle('hidden',step===4);$('#payBtn').classList.toggle('hidden',step!==4);$('#payBtn').textContent=`Pay $${depositAmount()} deposit & confirm`;if(step===2&&state.service)renderQuickDates();if(step===4)renderSummary();clearError();scrollTo({top:0,behavior:'smooth'})}
